...
import { QueryPattern, MatchType, extractPatterns } from '../extract/index';
...
export async function getDigestUsage(): Promise<DigestUsage | null> {
  try {
    ...
    const response = await fetch(`${SERVER_URL}/digest/usage?email=${encodeURIComponent(auth.email)}`, {
      headers: {
